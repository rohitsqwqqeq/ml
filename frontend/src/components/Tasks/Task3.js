  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/api/ball/route3?year=${year}`);
        // console.log(response);
        setTeams(response.data.teams);
        setExtraRuns(response.data.extraRuns);
        // console.log(response.data.teams);
        // console.log(response.data.extraRuns);
      } catch (error) {
        console.error('Error fetching data:', error);
      }
    };
    ...
    datasets: [
      {
        label: 'Extra Runs',
        data: extraRuns,
        backgroundColor: generateRandomColors(teams.length),
  ...
  return (
    <div style={{ display:'flex', flexDirection:'row' }}>
      ...
      <div style={{ textAlign: 'center', paddingTop: '40px' }}>
        <h4>Fig 3: Extra Runs conceded by each Teams in year {selectedYear}</h4>
      </div>
      </div>
    </div>
  );
